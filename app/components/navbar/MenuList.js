...
const MenuList = ({ show, handleShow, isDarkMode }) => {
  ...
  const { logged, userLogout } = useAuth();
    ...
    <div className={`${show ? "opacity-100 visible" : "opacity-0 invisible"} transition-all fixed inset-0 bg-black/50 flex justify-start z-20`}>
      <aside className={`${!show ? "translate-x-48" : ""} transition-all w-72 ${isDarkMode ? "bg-orange-700" : "bg-green-700"}`}>
        ...
        <ul style={{ height: 'calc(100vh - 72px)' }} className='flex flex-col gap-4 px-3 pb-10 justify-evenly items-center text-center'>
          { logged ? <Link href={"/views/auth/users/:id"} className={`text-white p-2 hover:text-gray-700 font-bold`}><Image src={"/user-circle-svgrepo-com.svg"} height={50} width={50} alt="profile" /></Link> : "" }
          ...
          { logged ? "" : <Link href={"/views/auth/register"} className={`text-white p-2 hover:text-gray-700 font-bold ${path === "/views/auth/register" ? "underline" : "no-underline"}`}>Register</Link> }
          { logged ? "" : <Link href={"/views/auth/login"}><div className={`text-white p-2 hover:text-gray-700 font-bold ${path === "/views/auth/login" ? "underline" : "no-underline"}`}>Login</div></Link> }
          { logged ? <p onClick={userLogout} className={`text-white p-2 hover:text-gray-700 font-bold`}>Salir</p> : "" }
        </ul>
      </aside>
    </div>
  )
